...
export const FIELD_HELP: Record<string, string> = {
  ...
  "gateway.controlUi.toolTitles":
    "Opt-in AI purpose titles for tool calls in Control UI chat (default off). When enabled, the chat.toolTitles method generates short titles for complex tool calls with the agent's utilityModel (or the low-cost openai/gpt-5.6-luna default when the agent already runs on OpenAI) and caches them in the per-agent state database. Leave off to keep tool rendering fully deterministic with no background model calls.",
